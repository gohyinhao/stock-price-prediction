...
X_training = data_processor.X_training / NORMALIZATION_CONSTANT
Y_training = data_processor.Y_training / NORMALIZATION_CONSTANT

X_validation = data_processor.X_validation / NORMALIZATION_CONSTANT
...
model.add(tf.keras.layers.Dense(NUM_OF_NODES, activation=tf.nn.leaky_relu))
model.add(tf.keras.layers.Dense(NUM_OF_NODES, activation=tf.nn.leaky_relu))
...
plt.title('Gold 1-day Price Forecast (MLP)')
plt.xlabel('Time')
plt.ylabel('Price')
plt.legend()
plt.show()
